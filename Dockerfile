...
WORKDIR /build
RUN apt-get update && apt-get install xz-utils curl git -y
...
RUN mkdir -p /arduino-ide/hardware/esp8266com && \
    git clone https://github.com/esp8266/Arduino.git /arduino-ide/hardware/esp8266com/esp8266 && \
    cp -r /arduino-ide/hardware/esp8266com/esp8266/libraries /arduino-ide/libraries


FROM launcher.gcr.io/google/debian9
WORKDIR /sketch
COPY --from=builder /build/arduino-ide /ide
